var $builtinmodule = function (name) {
    ...
    Sk.simplegui.cleanup = function () { // Cleanup the module - kill all timers, release all resources
        for (var i = 0; i < timers.length; i++) { // Stop the timer, if it's running
            Sk.misceval.callsim(timers[i].stop, timers[i]);
        };
        timers = [];
        for (var i = 0; i < sounds.length; i++) { // Stop the sound, if it's playing
            Sk.misceval.callsim(sounds[i].pause, sounds[i]);
        };
        sounds = [];
        for (i = 0; i < frames.length; i++) { // Close the window, if it's open
            Sk.misceval.callsim(frames[i].stop, frames[i]);
        };
        frames = [];
    };
    // goog.require(\"goog.events\");    // goog.require(\"goog.events.KeyCodes\");    // goog.require(\"goog.events.KeyHandler\");
    // Images
    ...
    function base64_encode(data) { // http://kevin.vanzonneveld.net
        // +   original by: Tyler Akins (http://rumkin.com)        // +   improved by: Bayron Guevara
        // +   improved by: Thunder.m
        // +   improved by: Kevin van Zonneveld (http://kevin.vanzonneveld.net)        // +   bugfixed by: Pellentesque Malesuada
        // +   improved by: Kevin van Zonneveld (http://kevin.vanzonneveld.net)        // +   improved by: Rafa\u0142 Kukawski (http://kukawski.pl)        // *     example 1: base64_encode('Kevin van Zonneveld');        // *     returns 1: 'S2V2aW4gdmFuIFpvbm5ldmVsZA=='        // mozilla has this native
        // - but breaks in 2.0.0.12!        //if (typeof this.window['btoa'] == 'function') {        //    return btoa(data);        //}        var b64 = \\"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=\";        var o1, o2, o3, h1, h2, h3, h4, bits, i = 0,        ac = 0,        enc = \"\",        tmp_arr = [];        
        if (!data) {
            return data;
        }
    ...
    mod.create_sound = new Sk.builtin.func(function (sound_data, sample_rate, num_channels) {
        ...
        // Header
        var data = [0x52, 0x49, 0x46, 0x46, // ChunkID: \\"RIFF\\"            getByte(chunksize, 0),            getByte(chunksize, 1),            getByte(chunksize, 2),            getByte(chunksize, 3),   // ChunkSize: 36 + SubChunk2Size
            0x57, 0x41, 0x56, 0x45, // Format: \\"WAVE\\"            0x66, 0x6d, 0x74, 0x20,  // Subchunk1ID: \\"fmt \\"            0x10, 0x00, 0x00, 0x00,  // Subchunk1Size: 16
            0x01, 0x00, // AudioFormat: 1 (PCM)            getByte(num_channels, 0), 
            getByte(num_channels, 1), // NumChannels
            getByte(sample_rate, 0), getByte(sample_rate, 1), getByte(sample_rate, 2), getByte(sample_rate, 3), // SampleRate
            getByte(byteRate, 0), getByte(byteRate, 1), getByte(byteRate, 2), getByte(byteRate, 3), // ByteRate
            getByte(blockAlign, 0), getByte(blockAlign, 1), // BlockAlign
            getByte(bitsPerSample, 0), getByte(bitsPerSample, 1), // BitsPerSample
            0x64, 0x61, 0x74, 0x61, // Subchunk2ID: \\"data\\"            getByte(subchunk2size, 0), 
    ...
    var canvas = function ($gbl, $loc) {
        ...
        $loc.draw_image = new Sk.builtin.func(function (self, image, srcpos,
            ...
            if ((srcx < 0) || (srcy < 0)) { // Do not draw anything
                return;
            };
            if ((srcx + srcw > im.width) || (srcy + srch > im.height)) { // Safari: Do not draw anything
                return;
            };
            self.context.save();
            ...
            self.context.drawImage(im,
                srcx, srcy, srcw, srch, dstoffx, dstoffy, dstw, dsth);
    ...
    var create_canvas = function (win, width, height, border) {
        ...
        canvas.onselectstart = function () { // Prevent weird selection on double click
            return false;
        };
        return canvas;
    };
    ...
    var canvasCoords = function (canvas, evt) { // Get canvas position
        var curr = canvas;
        var top = 0;
        var left = 0;
    ...
    code2key[37] = String.fromCharCode(0x2190); // \\"left\";    code2key[38] = String.fromCharCode(0x2191); // \\"up\";  
    code2key[39] = String.fromCharCode(0x2192); // \\"right\";    code2key[40] = String.fromCharCode(0x2193); // \\"down\";    
    // Python frame class
    var frame = function ($gbl, $loc) {
        $loc.__init__ = new Sk.builtin.func(function (self, title,
            ...
            self.frame_window = window.open('', 'myframe', 'width=' + winwidth + ',height=' + winheight + ',location=0' + ',menubar=0' + ',toolbar=0' + ',directories=0' + ',status=0' + ',titlebar=0' + ',scrollbars=0' + ',resizeable=1');
            ...
            // Control Pane
            self.control = self.frame_window.document.createElement(\"div\"); // +4 to match 2px border around canvas
            ...
            var mousemove = function (evt) {
                var coords = canvasCoords(self.canvas, evt); // Adjust position to remove border
            ...
            framediv.style.margin = self.margin + \\"px\"; // framediv.style.overflow = \\"auto\";            framediv.appendChild(self.control);            framediv.appendChild(self.canvas);
        ...
        $loc.start = new Sk.builtin.func(function (self) {
            ...
            self.frame_window.onbeforeunload = function () { // Works for Chrome and FireFox
                cancelAnimation(self);
            };
            self.frame_window.onunload = function () { // Needed for Safari
                cancelAnimation(self);
            };
