# Simple implementation of GalaxyInvaders game
# by Igor Petetskih (Russia), Fall 2013, Coursera
#
# Ships images by Artem Belov (https://www.fl.ru/users/Wedmak2/viewproj.php?prjid=2988856)
# music downloaded from http://keygenjukebox.com/
# sounds generated in http://www.bfxr.net/ service
# for background I got a free wallpaper that I found in internet
# backgrounds links:
# http://www.ehdwalls.com/1024x768/sf/nasa_-_cone_nebula__ngc_2264/
# http://www.ehdwalls.com/1024x768/sf/nasa_-_cartwheel_galaxy__head-on_collision/
# http://www.ehdwalls.com/1024x768/sf/nasa_-_the_andromeda_galaxy__m31__spyral_galaxy/
# http://www.ehdwalls.com/1024x768/sf/nasa_-_mars__dust_storms__2001/
# http://www.ehdwalls.com/1024x768/sf/shuttle_view_of_the_sunshine_state__florida/
# http://www.ehdwalls.com/1024x768/sf/nasa_-_the_glowing_eye_nebula__ngc_6751/
# http://www.allwall4u.com/desktop-wallpaper-space/

VER = "1.1.3"
ver_date = "21 dec. 2013"
...
PREFIX = codeskulptor.file2url('demos-')[:-6] + "GalaxyInvaders/"

# Images:
#pImage = simplegui.load_image('https://dl.dropbox.com/s/zhnjucatewcmfs4/player.png')
pImage = simplegui.load_image(PREFIX + 'player.png')

aImages = []
for i in range(7):
    aImages.append([])
aImages[0].append(simplegui.load_image(PREFIX + 'alien_1_1.png'))
aImages[0].append(simplegui.load_image(PREFIX + 'alien_1_2.png'))
aImages[0].append(simplegui.load_image(PREFIX + 'alien_1_3.png'))

aImages[1].append(simplegui.load_image(PREFIX + 'alien_2_1.png'))
aImages[1].append(simplegui.load_image(PREFIX + 'alien_2_2.png'))
aImages[1].append(simplegui.load_image(PREFIX + 'alien_2_3.png'))

aImages[2].append(simplegui.load_image(PREFIX + 'alien_3_1.png'))
aImages[2].append(simplegui.load_image(PREFIX + 'alien_3_2.png'))
aImages[2].append(simplegui.load_image(PREFIX + 'alien_3_3.png'))

aImages[3].append(simplegui.load_image(PREFIX + 'alien_4_1.png'))
aImages[3].append(simplegui.load_image(PREFIX + 'alien_4_2.png'))
aImages[3].append(simplegui.load_image(PREFIX + 'alien_4_3.png'))

aImages[4].append(simplegui.load_image(PREFIX + 'alien_5_1.png'))
aImages[4].append(simplegui.load_image(PREFIX + 'alien_5_2.png'))
aImages[4].append(simplegui.load_image(PREFIX + 'alien_5_3.png'))

aImages[5].append(simplegui.load_image(PREFIX + 'alien_6_1.png'))
aImages[5].append(simplegui.load_image(PREFIX + 'alien_6_2.png'))
aImages[5].append(simplegui.load_image(PREFIX + 'alien_6_3.png'))

aImages[6].append(simplegui.load_image(PREFIX + 'alien_7_1.png'))
aImages[6].append(simplegui.load_image(PREFIX + 'alien_7_2.png'))
aImages[6].append(simplegui.load_image(PREFIX + 'alien_7_3.png'))

#backgrounds
bckg = [simplegui.load_image(PREFIX + "back01.jpg"),
        simplegui.load_image(PREFIX + "back02.jpg"),
        simplegui.load_image(PREFIX + "back03.jpg"),
        simplegui.load_image(PREFIX + "back04.jpg"),
        simplegui.load_image(PREFIX + "back05.jpg"),
        simplegui.load_image(PREFIX + "back06.jpg"),
        simplegui.load_image(PREFIX + "back07.jpg")]

# sounds
sndPlayer = simplegui.load_sound(PREFIX + 'player_shoot.wav')
sndAlien = simplegui.load_sound(PREFIX + 'alien_shoot.wav')
sndPlayerExplosion = simplegui.load_sound(PREFIX + 'explosion%2001.wav')
sndAlienHit = simplegui.load_sound(PREFIX + 'alien_hit.wav')
sndAlienExplosion = simplegui.load_sound(PREFIX + 'explosion_02.wav')
sndBonus = simplegui.load_sound(PREFIX + 'bonus.wav')
sndPause = simplegui.load_sound(PREFIX + 'pause.wav')
sndTheme = simplegui.load_sound(PREFIX + 'theme_01.mp3')
...
class Alien:
    ...
    def draw(self, canvas):
        ...
        if self.health != self.get_max_health():
            ...
            canvas.draw_line([self.x - w // 2, self.y - h // 2 - 3], [self.x - w // 2 + ratio, self.y - h // 2 - 3], 4,
                             "green")
        return canvas
...
class AliensFleet:
    def __init__(self, point):
        ...
        def is_high_level(pl):
            map_ = (level - 1) % 7
            row = level_map[map_][pl[1]]
            return (row & (1 << pl[0])) != 0
        ...
        for i in xrange(self.get_aliens_count()):
    ...
    def move_side(self):
        ...
        for i in range(len(self.aliens)):
            ...
            if point[0] > right:
                right = point[0]
        if (self.x + (left + 1) * 60 < LEFT_MARGIN + 10) or (
                    self.x + (right + 1) * 45 > FIELD_WIDTH - LEFT_MARGIN - 60):
    ...
    def alien_fly(self):
        ...
        for alien in self.aliens:
            if alien.flying:
                i += 1
            if (i < 1 + level) and (random.randrange(1000) < 3) and (
                        time.time() - level_time[len(level_time) - 1] > 60):
                alien.flying = True
    ...
    def check_death(self):
        ...
        for bullet in user_bullet:
            for i in xrange(len(self.aliens)):
                alien = self.aliens[i]
                if isBulletHit(bullet, alien):
                    if alien.health - player.power <= 0:
                        ...
                        x = ~int((1 << point[0]))
                        self.pattern[point[1]] &= x
                        user_del_bullet.append(bullet)
        ...
        for bullet in user_del_bullet:
            if bullet in user_bullet:
...
class Player:
    ...
    def explode(self):
        ...
        global alien_fleet, player_killed_at, player_killed, player_speed, bonuses
        player_speed = 0
...
def draw_start_screen(canvas):
    ...
    canvas.draw_text(ver_date, [600, 190], 20, "yellow")

    draw_text(canvas, "CONTROLS:", [110, 210], 24, [2, 2], ["green", "yellow"])

    draw_text(canvas, "Arrows - to left and right, space - to fire, P to pause game", [110, 240], 24, [2, 2],
              ["green", "yellow"])
...
def draw_end_screen(canvas):
    ...
    for bc in bonus_count:
        ...
        b.draw(canvas)
        draw_text(canvas, " - used " + str(bonus_count[i]) + " times", [530, 320 + 40 * i], 24, [2, 2],
                  ["blue", "yellow"])
        i += 1
...
def isBulletHit(bullet, obj):
    if (bullet.y + bullet.height // 2 + 2 > obj.y - obj.height // 2) and (
                    bullet.y - bullet.height // 2 - 2 < obj.y + obj.height // 2):
        if (bullet.x + bullet.width // 2 + 2 > obj.x - obj.width // 2) and (
                        bullet.x - bullet.width // 2 - 2 < obj.x + obj.width // 2):
            return True
        else:
            return False
    else:
...
def new_level():
    ...
    level_time.append(time.time())
    current_back = random.randrange(len(bckg))
    level += 1
    player_speed = 0
    user_bullet = []
    alien_bullets = []
